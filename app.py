...
def get_portfolios():
    ...
    sql_query = '''
        SELECT p.*, u.username as owner_username,
               ...
               EXISTS(SELECT 1 FROM likes WHERE portfolio_id = p.id AND user_id = %s) as is_liked
        FROM portfolios p
    ...
    params = []
    if search_query:
        sql_query += 'WHERE p.portfolio_title ILIKE %s OR u.username ILIKE %s OR p.description ILIKE %s'
        search_term = f"%{search_query}%"
        params.extend([search_term, search_term, search_term])
    sql_query += ' GROUP BY p.id, u.username ORDER BY p.upload_date DESC'
    current_user_id = session.get('user_id')
    final_params = [current_user_id] + params
...
def profile(username):
    ...
    cur.execute('SELECT * FROM portfolios WHERE user_id = %s ORDER BY upload_date DESC', (user['id'],))
...
@login_required
def like_portfolio(id):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('SELECT * FROM likes WHERE user_id = %s AND portfolio_id = %s', (g.user['id'], id))
    like = cur.fetchone()
    if like:
        cur.execute('DELETE FROM likes WHERE user_id = %s AND portfolio_id = %s', (g.user['id'], id))
        liked = False
    else:
        cur.execute('INSERT INTO likes (user_id, portfolio_id) VALUES (%s, %s)', (g.user['id'], id))
        liked = True
    conn.commit()
    cur.execute('SELECT COUNT(*) FROM likes WHERE portfolio_id = %s', (id,))
